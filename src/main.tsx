...
async function enableMocking() {
  ...
  return worker.start({
    serviceWorker: {
      url: `${import.meta.env.BASE_URL}mockServiceWorker.js`, // 서비스 워커 URL 설정
      options: { updateViaCache: 'none' },  // 항상 최신 서비스 워커 사용 (캐시 무시)
    },
    // API 요청만 경고, 내비게이션·정적 에셋은 조용히 통과
    onUnhandledRequest(req, print) {
      if (req.url.includes('/api/')) {
        print.warning()  // MSW 표준 경고 출력
      }
      // 그 외(정적 에셋, 내비게이션 등)는 조용히 통과
    },
